...
static uint16_t velocity_to_step_delay(int32_t velocity_steps_per_sec, float pio_freq);
...
void motor_set_position(motor_system_t* sys, uint8_t axis, int32_t target, uint16_t max_speed) {
    ...
    if (position_diff != 0) {
        int32_t move_velocity = (position_diff > 0) ? max_speed : -max_speed;
        motor_set_velocity(sys, axis, move_velocity);
        printf("Axis %d goto: current=%d, target=%d, velocity=%d\n", 
               axis, (int)ax->position, (int)target, (int)move_velocity);
    }
}
...
void motor_set_pvt_target(motor_system_t* sys, uint8_t axis, int32_t target_pos, int32_t target_vel, uint64_t target_time) {
    ...
    printf("Axis %d PVT target: pos=%d, vel=%d, time=%llu\n", 
           axis, (int)target_pos, (int)target_vel, target_time);
}
...
void motor_apply_correction(motor_system_t* sys, uint8_t axis, int16_t correction) {
    ...
    motor_axis_t* ax = &sys->axes[axis];
    int32_t corrected_velocity = ax->target_velocity + correction;
    
    motor_set_velocity(sys, axis, corrected_velocity);
    printf("Axis %d correction: %d steps, new velocity: %d\n", 
           axis, correction, (int)corrected_velocity);
}
...
void update_step_rate(step_generator_t* gen, int32_t velocity, float sys_clock) {
    ...
    uint16_t step_delay = velocity_to_step_delay(abs(velocity), pio_freq);
...
static uint16_t velocity_to_step_delay(int32_t velocity_steps_per_sec, float pio_freq) {
    ...
    return calculate_step_delay((float)velocity_steps_per_sec, pio_freq);
}
...
static void feed_step_fifo(motor_system_t* sys, uint8_t axis) {
    ...
    while (pio_sm_get_tx_fifo_level(gen->pio, gen->sm) < queue_depth) { // (!pio_sm_is_tx_fifo_full(gen->pio, gen->sm)) {
        filled = true;
        if (delay == 0xFFFF)
        {
            return;
...
void motor_update_task(motor_system_t* sys) {
    ...
    for (uint8_t i = 0; i < NUM_AXES; i++) {
        ...
        if (ax->moving && ax->target_position != 0) {
            ...
            int32_t new_vel = ax->target_velocity + ((ax->p_gain * pos_err) + (ax->i_gain * error_integral[i]) + (ax->d_gain * derivative_error)) / 1000.0;
            ...
            int32_t vel_change = new_vel - ax->velocity;
            int32_t max_change = (MAX_ACCELERATION * delta_t);
            if (ax->velocity == 0 && abs(vel_change) > STARTUP_VELOCITY)
            {
                new_vel = (vel_change > 0 ? STARTUP_VELOCITY : -STARTUP_VELOCITY);
            } else if (abs(vel_change) > max_change) {
            ...
            //
            // if (abs(ax->target_velocity - new_vel) < 10 && fabs(pos_err) < 1)
            // {
            //     printf("Finished pid move\r\n");
            //     motor_set_velocity(g_motor_system, i, ax->target_velocity);
            //     ax->target_position = 0;

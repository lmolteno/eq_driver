...
#define USE_USB_CDC         1   // 1 = USB CDC, 0 = UART1
...
typedef struct {
    int32_t ra_velocity;   // steps/sec (signed for direction)
    int32_t dec_velocity;  // steps/sec (signed for direction)
} __attribute__((packed)) set_velocity_msg_t;

typedef struct {
    int32_t ra_target;     // target step position
    int32_t dec_target;    // target step position
    uint16_t max_speed;    // max steps/sec during move
} __attribute__((packed)) set_position_msg_t;

typedef struct {
    uint8_t axis;          // 0=RA, 1=DEC
    int16_t correction;    // correction in steps
} __attribute__((packed)) correction_msg_t;
